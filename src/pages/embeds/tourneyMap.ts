...
export const tourneyMapEmbed = (data: BeatmapResponseApi, pp: number, sheetId: string, mapName: string, mapStats: MapStats, starRating: number): InteractionReplyOptions => {
    const file = new AttachmentBuilder(`./src/public/${sheetId}.png`)
    const embed = new EmbedBuilder()
        ...
        .addFields({
            name: `Beatmap Info`,
            value: `${starRating.toFixed(2)}⭐️ - ${data.BPM * mapStats.speedMultiplier}bpm - ${sheetId === "DT" ? secondsToMinutes(data.TotalLength * 0.66) : secondsToMinutes(data.TotalLength)}⏱️ - x/${data.MaxCombo
                } combo \n CS ${mapStats.cs!.toFixed(1)} | AR ${mapStats.ar!.toFixed(1)} | OD ${mapStats.od!.toFixed(1)} | HP ${mapStats.hp!.toFixed(1)}`,
        })
