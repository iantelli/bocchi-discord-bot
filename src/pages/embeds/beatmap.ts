...
export const beatmapEmbed = (data: BeatmapResponseApi): InteractionReplyOptions => {
  const embed = new EmbedBuilder()
    ...
    .addFields({
      name: `Beatmap Info`,
      value: `${data.DifficultyRating}⭐️ - ${data.BPM}bpm - ${secondsToMinutes(data.TotalLength)}⏱️ - x/${data.MaxCombo} combo \n CS ${
        data.CS
      } | AR ${data.AR} | HP ${data.HP} | OD ${data.OD}`
    })
    .setFooter({ text: `Bocchi Bot - Powered by Kitsu API` })
    .setTimestamp(new Date())

  const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder().setURL(`${keys.apiLink}/d/${data.ParentSetID}`).setLabel("Download Map").setStyle(ButtonStyle.Link)
  )
  return {
    embeds: [embed],
    components: [row]
