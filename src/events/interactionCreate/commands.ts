...
export default event("interactionCreate", async ({ log, client }, interaction) => {
  ...
  try {
    ...
    if (interaction.deferred) return interaction.editReply(EditReply.error(`An error occurred while executing this command :(`))

    return interaction.reply(Reply.error(`An error occurred while executing this command :( \n\`\`\`${err}\`\`\``))
  }
})
